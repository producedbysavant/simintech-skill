...
#: Регулярки, а не строки: список строк ловил ровно те словоформы, что в нём
#: записаны, и перефраз («CreateBlock их не поддерживает», «записи не создать»)
#: проходил зелёным, оставаясь тем же ложным утверждением.
REFUTED_CLAIM_PATTERNS = (
    re.compile(r"не созда(ются|ётся|ется)"),
...
#: Положительное утверждение рядом с перечнем: замер 2026-09-18 дал ненулевой
#: id, и проза обязана это сказать. Одного запрета мало — формулировку можно
#: «починить» так, что от замера в тексте не останется ничего.
PROVEN_CREATION_STEMS = ("создаёт", "создаются", "создает")

#: Кто отказывает. Запрет без актора оставляет агента с прежним выводом: он
#: видит перечень и причину, но не знает, что идти надо в библиотеку, а не в COM.
REFUSER_STEMS = ("библиотек",)
...
def test_skills_do_not_claim_refused_classes_cannot_be_created():
    ...
    Проверка держит три вещи сразу: опровергнутого утверждения нет, создание
    названо состоявшимся, и рядом стоит тот, кто отказывает. Любая из них по
    отдельности пропускает правдоподобную на вид неправду: запрет — перефраз в
    другой словоформе, причина без актора — потерю «кто отказывает», актор без
    положительного утверждения — текст, из которого убрали сам замер.

    Checkout simintech-code этой проверке не нужен: она читает только скиллы
    репозитория. Поэтому маркера `requires_code` здесь нет — иначе локальный
    прогон из CONTRIBUTING не исполнял бы её вовсе.
    """
    for skill in REFUSED_CLASSES_SKILLS:
        window = _refused_classes_window(_skill(skill)).lower()
        for pattern in REFUTED_CLAIM_PATTERNS:
            found = pattern.search(window)
            assert found is None, (
                f"{skill}: формулировка «{found.group(0)}» опровергнута замером "
                "2026-09-18 — CreateBlock создаёт обе записи"
            )
        assert any(stem in window for stem in PROVEN_CREATION_STEMS), (
            f"{skill}: рядом с перечнем не сказано, что создание состоялось, — "
            "агент остаётся с выводом «создать нельзя»"
        )
        assert any(stem in window for stem in REFUSER_STEMS), (
            f"{skill}: рядом с перечнем не назван тот, кто отказывает "
            "(библиотека), — агент снова пойдёт в COM"
        )
...
#: Формы перечислены вариантами, а не одной строкой: «cannot create» не ловит
#: «can't create», и перефраз описания проходил зелёным (проверено мутацией).
REFUTED_DESCRIPTION_CLAIMS = (
    "cannot be created",
    "cannot create",
    "can't create",
    "cant create",
